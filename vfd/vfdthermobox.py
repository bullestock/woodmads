...
standoff3 = (res
             .faces(">Y")
             .workplane()
             .transformed(offset=(-20, -23, -(th+standoff_h)))
...
if not TOP:
    res = (res
           # Holes for 3 x motor fan
           .faces(">Y")
           .workplane(centerOption='CenterOfMass')
           .tag("gland")
           .transformed(offset=(-lv_x_dist, lv_hole_offset, 0), rotate=(0, 0, 90))
           .rarray(0, lv_x_dist, 1, 3)
           .slot2D(lv_hole_d, lv_hole_d /2)
           .cutBlind(-th)
           # Hole for thermo sensor
           .faces(">Y")
           .workplane(centerOption='CenterOfMass')
           .transformed(offset=(2*lv_x_dist, lv_hole_offset, 0), rotate=(0, 0, 90))
           .slot2D(2*3.2, 3.2)
           .cutBlind(-th)
           # Hole for pump power
           .faces(">Y")
...
if TOP:
    p = res.faces(">Z").workplane(-cut_h).split(keepTop=True)
else:
    p = res.faces(">Z").workplane(-cut_h).split(keepBottom=True)
show_object(p)
